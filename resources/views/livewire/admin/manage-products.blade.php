<div>
  <div>
    <div wire:poll.10s>
      @if ($unconfirmed_products && count($unconfirmed_products) > 0)
        <p class="relative my-4 mb-8 pb-1 text-center text-lg font-bold text-slate-800 dark:text-slate-200">
          محصولات تایید نشده
          <span
            class="absolute bottom-0 right-1/2 translate-y-full translate-x-1/2 w-28 h-1 bg-slate-800 dark:bg-slate-200 rounded-full"></span>
        </p>
        <div class="overflow-x-auto">
          <table class="w-full text-sm text-left rtl:text-right shadow-md text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              @foreach ($unconfirmed_products as $product)
                <tr wire:key="unconfirmed-product-{{ $product->id }}"
                  ...
                  <td scope="row" class="align-middle px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                    <div class="flex items-center justify-start w-max">
                      <img class="w-10 h-10 rounded-full"
                        src="{{ route('user-avatar', ['user' => $product->user->id]) }}"
                        alt="{{ $product->user->name() }}">
                      <div class="ps-3">
                        <a href="{{ route('admin.user.products', ['user' => $product->user->id]) }}"
                          class="block text-base font-semibold hover:text-primary hover:underline">{{ $product->user->name() }}</a>
                        <div class="font-normal text-gray-500">{{ $product->user->email }}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="align-middle whitespace-nowrap px-6 py-4">
                    <a href="{{ route('admin.products.edit', ['slug' => $product->slug]) }}"
                      ...
                      <span class="text-xs ps-1 border-s border-blue-800">ویرایش و جزئیات</span>
                    </a>
                    <button wire:key="confirm-product-{{ $product->id }}"
                      wire:click="confirmProduct('{{ $product->slug }}')"
                      ...
                      <svg wire:loading.remove.flex wire:target="confirmProduct('{{ $product->slug }}')"
                        xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" class="lucide lucide-shield-check pe-1">
                        <path
                          d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z" />
                        <path d="m9 12 2 2 4-4" />
                      </svg>
                      <span wire:loading.flex wire:target="confirmProduct('{{ $product->slug }}')" class="flex pe-1">
                        <x-spinner size="[15px]" color="white" />
                      </span>
                      <span class="text-xs ps-1 border-s border-lime-800">تایید محصول</span>
                    </button>
                    <button wire:key="cancel-product-{{ $product->id }}"
                      wire:click="cancelProduct('{{ $product->slug }}')"
                      ...
                      <svg wire:loading.remove.flex wire:target="cancelProduct('{{ $product->slug }}')"
                        xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" class="lucide lucide-shield-alert pe-1">
                        <path
                          d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z" />
                        <path d="M12 8v4" />
                        <path d="M12 16h.01" />
                      </svg>
                      <span wire:loading.flex wire:target="cancelProduct('{{ $product->slug }}')" class="flex pe-1">
                        <x-spinner size="[15px]" color="white" />
                      </span>
                      <span class="text-xs ps-1 border-s border-red-800">لغو محصول</span>
                    </button>
                  </td>
                </tr>
              @endforeach
      ...
      <div dir="ltr" class="my-8">
        {{ $unconfirmed_products->links(data: ['scrollTo' => false]) }}
      </div>
      ...
      <div
        class="flex items-baseline sm:items-end justify-between flex-column flex-wrap flex-col sm:flex-row pb-4 mt-8">
