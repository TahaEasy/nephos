<x-auth-head regForm>
  <form action="/auth/register" method="POST" class="xs:min-w-96 min-w-0 xs:w-auto w-full px-2 mb-6">
    @csrf
    <div>
    ...
    <div class="mt-4">
      ...
      <div class="relative text-right w-full">
        <input type="text" autocomplete="off" name="f_name" id="f_name" value="{{ old('f_name') }}"
          class="peer outline-none rounded-sm @error('f_name') border-red-600 @else dark:border-gray-700 focus:dark:border-gray-300 border-gray-300 hover:border-gray-400 focus:border-gray-700 @enderror dark:bg-slate-800 dark:text-slate-200 border-[1px] w-full py-2 pe-2 ps-12 transition-all duration-200">
        <span
          class="absolute right-4 top-1/2 -translate-y-1/2 @error('f_name') text-red-600 @else text-gray-300 @enderror peer-focus:text-primary ransition-all duration-200">
    ...
    <div class="mt-4">
      <div class="flex justify-between">
        <label for="l_name" class="block text-right text-sm text-slate-600 dark:text-gray-400 mb-1 font-semibold">نام
          خانوادگی</label>
        @error('l_name')
          <p class="text-xs text-red-600">{{ $message }}</p>
        @enderror
      ...
      <div class="relative text-right w-full">
        <input type="text" autocomplete="off" name="l_name" id="l_name" value="{{ old('l_name') }}"
          class="peer outline-none rounded-sm @error('l_name') border-red-600 @else dark:border-gray-700 focus:dark:border-gray-300 border-gray-300 hover:border-gray-400 focus:border-gray-700 @enderror dark:bg-slate-800 dark:text-slate-200 border-[1px] w-full py-2 pe-2 ps-12 transition-all duration-200">
        <span
          class="absolute right-4 top-1/2 -translate-y-1/2 @error('l_name') text-red-600 @else text-gray-300 @enderror peer-focus:text-primary ransition-all duration-200">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            class="feather feather-user">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
    ...
    <div class="mt-4">
      ...
      <div class="relative text-right w-full">
        <input type="password" name="password" id="password"
          class="password-input peer outline-none rounded-sm @error('password') border-red-600 @else dark:border-gray-700 focus:dark:border-gray-300 border-gray-300 hover:border-gray-400 focus:border-gray-700 @enderror dark:bg-slate-800 dark:text-slate-200 border-[1px] w-full py-2 pe-2 ps-12 transition-all duration-200">
        <span
    ...
    <div class="mt-4">
      <div class="flex justify-between">
        <label for="password-confirmation"
          class="block text-right text-sm text-slate-600 dark:text-gray-400 mb-1 font-semibold">تکرار رمز
          عبور</label>
      ...
      <div class="relative text-right w-full">
        <input type="password" name="password_confirmation" id="password_confirmation"
          class="password-input peer outline-none rounded-sm @error('password_confirmation') border-red-600 @else dark:border-gray-700 focus:dark:border-gray-300 border-gray-300 hover:border-gray-400 focus:border-gray-700 @enderror dark:bg-slate-800 dark:text-slate-200 border-[1px] w-full py-2 pe-2 ps-12 transition-all duration-200">
        <span
          class="absolute right-4 top-1/2 -translate-y-1/2 @error('password_confirmation') text-red-600 @else text-gray-300 @enderror peer-focus:text-primary ransition-all duration-200">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            class="feather feather-lock">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2">
    ...
    <div class="flex justify-between items-center my-4">
      <p class="text-gray-400 font-medium">اشتراک خبر نامه؟</p>
      <div class="relative bg-zinc-300 dark:bg-slate-500 w-14 h-7 rounded-full p-1">
        <input type="checkbox" id="offer-toggler" class="relative peer w-full h-full opacity-0 cursor-pointer z-10">
        <span
    ...
    <div>
      <button
        class="block py-3 px-8 w-full text-sm text-white bg-primary hover:bg-primary/80 glow-primary glow-hover rounded-full transition-all duration-300">ثبت
        نام</button>
    </div>
  </form>
</x-auth-head>
