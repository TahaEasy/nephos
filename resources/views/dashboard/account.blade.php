<x-dashboard-layout header="حساب کاربری">
    ...
    <div class="col-span-2">
      <div
        ...
        <div class="relative bg-slate-50 dark:bg-slate-700/40 border-t-[1px] border-t-gray-400/50 w-full p-4">
          <p class="text-xs text-gray-400">نــوع حــساب شــما:</p>
          <div class="flex justify-center">
            <p
              ...
              {{ auth()->user()->is_seller ? 'فروشنده' : 'مشتری' }}
            </p>
          </div>
          @if (auth()->user()->is_seller)
            <a href="{{ route('create-product') }}"
              class="absolute right-4 bottom-4 inline-block text-sm w-fit text-slate-800 dark:text-primary/80 hover:dark:text-primary hover:text-primary/80 transition-all duration-150">افزودن
              کالا+</a>
          @endif
        </div>
      </div>
      @if (auth()->user()->is_address_incomplete())
        <div class="flex justify-start items-center gap-2 mt-6 px-4 py-2 rounded-sm bg-slate-700 dark:bg-slate-900">
          ...
          <div>
            ...
            <p class="xl:text-xs text-xxs leading-normal mt-1 text-slate-200">اطلاعات بخش آدرس برای تکمیل سفارشات شما
              ضروری هستند و حتما باید کامل شده باشند تا بتوانید سفارش خود را به پایان برسانید!</p>
          </div>
        </div>
      @endif
      <div
        class="mt-6 p-4 py-3 bg-white dark:bg-slate-800 text-center rounded-sm hover:drop-shadow-xl w-full transition-all duration-300">
        <livewire:messages />
      </div>
    </div>
